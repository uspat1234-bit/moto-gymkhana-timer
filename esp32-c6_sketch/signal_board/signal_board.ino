/*
 * ====================================================================
 * MGTS Signal Board Core Unit
 * [機能]
 * - SEQ_STARTでカウントダウン(赤-3秒 -> 緑0秒)を実行
 * - START検知時、緑(0秒)より前ならFLYING、以降ならREACTIONをESP-NOWでハブへ通知
 * - 注目を集める強烈なフライング警告音を実装
 * ====================================================================
 */
...
IPAddress dns(192, 168, 1, 1); IPAddress gateway(192, 168, 1, 1); IPAddress subnet(255, 255, 255, 0);
byte mac[] = { 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0x30 }; const int UDP_PORT = 5005;            
...
Adafruit_NeoMatrix matrix = Adafruit_NeoMatrix(64, 8, LED_PIN, NEO_MATRIX_TOP + NEO_MATRIX_LEFT + NEO_MATRIX_COLUMNS + NEO_MATRIX_ZIGZAG, NEO_GRB + NEO_KHZ800);
EthernetUDP ethUdp; char packetBuffer[512]; 
...
void showStatus(String txt, uint16_t color) {
  matrix.setTextColor(color); int x = (64 - (txt.length() * 6)) / 2; matrix.setCursor(x >= 0 ? x : 0, 1); matrix.print(txt);
}

// 判定結果をハブへESP-NOWでのみ通知 (UDPブロードキャストはしない)
...
void setup() {
  ...
  for (int i = 0; i < 256; i++) sine_table[i] = (int16_t)(15000.0 * sin(2.0 * M_PI * i / 256.0)); // 音量ブースト
  pinMode(ETH_RST_PIN, OUTPUT); digitalWrite(ETH_RST_PIN, LOW); delay(100); digitalWrite(ETH_RST_PIN, HIGH); delay(500); 
  SPI.begin(SPI_SCK, SPI_MISO, SPI_MOSI, ETH_CS_PIN); Ethernet.init(ETH_CS_PIN); Ethernet.begin(mac, ip_eth, dns, gateway, subnet);
  if (Ethernet.hardwareStatus() != EthernetNoHardware) ethUdp.begin(UDP_PORT);
  WiFi.mode(WIFI_STA); esp_wifi_set_channel(1, WIFI_SECOND_CHAN_NONE); WiFi.disconnect();
  if (esp_now_init() == ESP_OK) { esp_now_register_recv_cb(OnDataRecv); esp_now_peer_info_t peer = {}; memcpy(peer.peer_addr, hubAddress, 6); peer.channel = 1; esp_now_add_peer(&peer); }
...
void updateDisplay() {
  ...
  if (currentSignalState == STATE_READY) {
    showStatus("READY", matrix.Color(0, 255, 255)); // 待機色はシアン
  } 
  else if (currentSignalState == STATE_FLYING) {
    matrix.fillScreen(c_red);
    showStatus("FLY", matrix.Color(255, 255, 255)); // 赤背景に白文字
  } 
  else {
...
void loop() {
  ...
  if (pEth) { int len = ethUdp.read(packetBuffer, 511); if (len > 0) { packetBuffer[len] = 0; processCommand(String(packetBuffer)); } }
