/*
 * ====================================================================
 * MGTS (Moto Gymkhana Timing System) - Goal Node Unit
 * * [概要]
 * 本プログラムは、モトジムカーナ計測システムにおける「ゴールセンサー部」の
 ...
 * ネットワーク構成設定
 * ==================================================================== */
// 有線LAN (W5500) 静的IPアドレス定義 (メイン: .20 / スタート: .21 と重複不可)
byte mac_eth[] = { 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xE2 };
IPAddress ip_eth(192, 168, 1, 22);      
...
// 宛先1：メインLEDボード（※実際のMACアドレスに書き換えてください）
uint8_t broadcastAddress1[] = {0x58, 0xE6, 0xC5, 0x12, 0xXX, 0xXX};
// 宛先2：シグナルボード等の2つ目の送信先（※実際のMACアドレスに書き換えてください）
uint8_t broadcastAddress2[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xYY, 0xYY}; 
...
void setup() {
  ...
  Serial.println("\n[INIT] Initialize MGTS Goal Node...");
  ...
  // ------------------------------------------------------------------
  WiFi.mode(WIFI_STA);
  esp_wifi_set_channel(1, WIFI_SECOND_CHAN_NONE); // チャンネルを1chに固定
  WiFi.disconnect();
    ...
    if (!isDetecting) {
      ...
      if (now - detectionStartTime >= DEBOUNCE_MS) {
        ...
        if (!hasTriggered && (now - lastSendTime > SEND_GUARD_MS)) {
          String msg = "STOP"; // ゴール判定のため STOP コマンドを定義
          
          Serial.println("\n-----------------------------------------");
          Serial.print("[TRIGGER] Goal line crossed. Duration: ");
          ...
          lastSendTime = now;
          hasTriggered = true; // 送信完了ロックを有効化（ゴールエリア滞留時の連続誤送信を抑制）
        }
      }
    }
  } else {
